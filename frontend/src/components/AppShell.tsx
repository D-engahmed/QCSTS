...
export default function AppShell({children}:{children:React.ReactNode}){
 const pathname=usePathname(),[open,setOpen]=useState(false);const {user,organization,site,logout,loading}=useAuth();
 ...
 <div className="tenant-switcher" aria-label="Assigned workspace"><Building2 size={16}/><div className="tenant-selects"><strong>{organization?.name||"Organization"}</strong><span>{site?.name||"Site"} · {user?.role||"Member"}</span></div></div>
 ...
 <div className="sidebar-footer"><span>Signed in as</span><strong>{user.full_name}</strong><small>{user.role||"Member"}</small></div></aside>
