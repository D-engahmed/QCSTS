  ...
  return (
    <div className="app-shell">
      ...
      <aside className={"sidebar " + (open ? "open" : "")}>
        ...
        <nav>
          <div className="nav-group">
            ...
            {workspace.map(([label, href, Icon]: any) => (
              <Link key={href} href={href} onClick={() => setOpen(false)} className={"nav-item " + (isActive(pathname, href) ? "active" : "")}>
        ...
        <div className="sidebar-footer"><span>Signed in as</span><strong>{user.full_name}</strong><small>{user.role || "Member"}</small></div>
      </aside>

      <main className="main-shell">
        <header className="topbar">
          <button className="mobile-menu" onClick={() => setOpen(true)} aria-label="Open navigation"><Menu size={19}/></button>
          <div className="global-search"><Search size={16}/><input placeholder="Search results, batches, products…" aria-label="Global search"/><kbd>Ctrl K</kbd></div>
          <div className="top-actions">
            <button className="icon-button" aria-label="Notifications"><Bell size={17}/></button>
            <div className="profile"><div className="avatar">{user.full_name.split(" ").map((part) => part[0]).slice(0, 2).join("").toUpperCase()}</div><div><strong>{user.full_name}</strong><span>{user.role || "Member"}</span></div></div>
