...
export default function Users() {
  ...
  const create=async(e:React.FormEvent)=>{e.preventDefault();setError("");try{await api(endpoints.users,{method:"POST",body:JSON.stringify(form)});setForm({email:"",full_name:"",role:"analyst",password:""});setOpen(false);await load()}catch(x){setError(x instanceof Error?x.message:"Unable to create member.")}};
  const edit=async(e:React.FormEvent)=>{e.preventDefault();if(!editing)return;setError("");try{await api(endpoints.user(editing.id),{method:"PATCH",body:JSON.stringify({full_name:form.full_name})});setEditing(null);setForm({email:"",full_name:"",role:"analyst",password:""});await load()}catch(x){setError(x instanceof Error?x.message:"Unable to update member.")}};
  ...
  return <div className="content">
    ...
    {editing&&<section className="card form-card member-form"><h3>Edit member</h3><form onSubmit={edit}><label className="field"><span>Full name</span><input required value={form.full_name} onChange={e=>setForm({...form,full_name:e.target.value})}/></label><div className="modal-actions"><button className="btn" type="button" onClick={()=>setEditing(null)}>Cancel</button><button className="btn primary">Save changes</button></div></form></section>}
    <section className="card table-card"><div className="card-header"><div><strong>{users.length} members</strong><span>{loading?"Loading authorized members…":"Live organization membership data"}</span></div><button className="btn" onClick={()=>void load()}>Refresh</button></div><div className="table-wrap"><table><thead><tr><th>Name</th><th>Email</th><th>Organization role</th><th>Status</th><th>Created</th><th>Actions</th></tr></thead><tbody>{users.map(u=><tr key={u.id}><td><b>{u.full_name}</b></td><td>{u.email}</td><td>{u.organization_role||"—"}</td><td><span className={"status "+(u.is_active?"active":"critical")}>{u.is_active?"Active":"Inactive"}</span></td><td>{new Date(u.created_at).toLocaleDateString()}</td><td><div className="action-row"><button className="btn" onClick={()=>{setEditing(u);setOpen(false);setForm({email:u.email,full_name:u.full_name,role:u.organization_role||"analyst",password:""})}}>Edit</button>{u.is_active&&<button className="btn reject" onClick={()=>void revoke(u)}>Revoke</button>}</div></td></tr>)}</tbody></table>{loading&&<div className="empty">Loading members…</div>}{!loading&&!users.length&&!error&&<div className="empty"><b>No members</b><span>No organization members were returned.</span></div>}</div></section>
  </div>;
}
