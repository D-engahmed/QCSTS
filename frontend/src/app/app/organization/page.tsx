...
export default function Organization() {
  const { organization, sites } = useAuth();
  ...
  return <div className="content">
    <div className="page-header"><div><span className="eyebrow">TENANT</span><h1>{organization?.name||"Organization"}</h1><p>Organization and site context is verified by the backend before protected requests are authorized.</p></div>{sites.length===0&&<button className="btn primary" onClick={()=>setOpen(true)}>Add site</button>}</div>
