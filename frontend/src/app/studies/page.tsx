...
export default function Studies() {
  ...
  useEffect(() => { load(); }, []);
  const rows = useMemo<Row[]>(() => studies.map((s) => ({ id: s.id, name: s.code + " · " + s.name, meta: [s.study_type, s.start_date ?? "No start date", s.target_end_date ? "End " + s.target_end_date : "No target end"].join(" · "), status: s.status.replaceAll("_", " "), tone: s.status === "active" ? "active" : s.status === "canceled" ? "critical" : "warning", extra: s.id })), [studies]);
